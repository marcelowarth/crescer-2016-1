...
$(function() {
  ...
  (function carregaImg(indice) {
    ...
    var nome = cavaleiro.nome;
    imgCavaleiro.src = cavaleiro.imagens[0].url;
    imgCavaleiro.alt = nome;
    ...
    imgCavaleiro.onload = function() {
      ...
      $img.on('click', function() {
        ...
        $detalhesCavaleiro.append($('<h3>').text( id ));
        $detalhesCavaleiro.append($('<h3>').text( nome ));
        $detalhesCavaleiro.append($('<h3>').text( altura / 100 ));
        $detalhesCavaleiro.append($('<button>').text("deletar " + nome).attr('name', 'btnDel').on('click', function(){
          //pegar posicao
          goldSaints.splice(id-1,1);
        }));
        $detalhesCavaleiro.append($('<h3>').text( 'Necessário recarregar página ao deletar' ));
      });
      if (indice < goldSaints.length - 1) carregaImg(indice + 1);
    };
  })(0);

  $('button[name=btnAddGolpe]').click(function() {
    var $label = $('<label>').text('Golpe: ').attr('for', 'golpe');
    var $golpe = $('<input>').attr('id', 'golpe').attr('type', 'text').attr('placeholder', 'Ex: Cólera do dragão').addClass('cadGolpe').attr('name', 'golpe');
    $('#golpe').append($('<li>').append($label).append($golpe));
  });


  $('button[name=btnAddImagem]').click(function() {
    ...
    $('#imgs').append($('<li>').append($novoTxt).append($novoCheckbox));
  });
...
function converterFormParaCavaleiro($form) {
  ...
  var imagens = buscaImagens();
  var golpes = buscaGolpes();

  var pesoConv = parseInt(formData.get('peso')) * 2.2046;
  var altConv = parseFloat(formData.get('altura').replace(",", ".")) * 100;
  ...
  function buscaImagens() {
    ...
    $('#imgs li').each(function(i) {
      var url = $(this).find('input[name=urlImagem]').val();
      var marcada = $(this).find('input[name=isThumb]').is(':checked');
      img.push({url: url, isThumb: marcada});
    });
    return img;
  };

  function buscaGolpes() {
    var golpes = [];
    $('#golpe li').each(function(i) {
      var golpe = $(this).find('input[name=golpe]').val();
      golpes.push(golpe);
    });
    return golpes;
  };
