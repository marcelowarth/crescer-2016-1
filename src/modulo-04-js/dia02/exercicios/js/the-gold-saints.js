...
function obterAlturaMedia() {
  var altTot = 0;
  for (var i in goldSaints) {
    altTot = altTot + goldSaints[i].alturaCm;
  }
  return parseFloat(((altTot/12)/100).toFixed(2));
}
...
function obterAlturaMediana() {
  ...
  if (alturas.length % 2) {
      var mediana = alturas[half];
  } else {
      var mediana = (alturas[half-1] + alturas[half]) / 2.0;
  };
