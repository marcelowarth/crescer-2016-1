...
var lbParaKg = function(lb) {
  return lb / 2.20462262;
};

var apenasComPeso = function(e) {
...
function obterCavaleiroComMaisGolpes() {
  var maisGolpes = goldSaints;
  return maisGolpes.concat().sort( function(a,b){
...
function obterAlturaMedia() {
  var cavaleiros = goldSaints;
  var altMedia = cavaleiros.reduce(function(acc, e){
    return acc + e.alturaCm;
  }, 0) / cavaleiros.length;
  return arredonda(cmParaMetros(altMedia));
}

function obterAlturaMediana() {
  var cavaleiros = goldSaints;
  cavaleiros = cavaleiros.concat().sort(function(a, b) {
    return a.alturaCm - b.alturaCm;
  });
  var centro = cavaleiros.length / 2;
  var mediana = (cavaleiros[centro].alturaCm + cavaleiros[centro - 1].alturaCm)/2;
  return arredonda(cmParaMetros(mediana));
}

function obterPesoMedio(cavaleiros){
  var cavComPeso = 0;
  var mediaPeso = (cavaleiros || goldSaints).filter(apenasComPeso)
                            .map(function(e){
                              cavComPeso++;
                              return e.pesoLb;
                            })
  ...
  return arredonda(lbParaKg(mediaPeso));
}
...
function obterIMC() {
  return goldSaints.filter(apenasComPeso)
                   .map(function(e) {
                     var imc = lbParaKg(e.pesoLb) / Math.pow(cmParaMetros(e.alturaCm), 2);
                     return arredonda(imc);
                   });
}
