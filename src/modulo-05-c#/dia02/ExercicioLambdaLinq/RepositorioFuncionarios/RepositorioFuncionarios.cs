...
namespace Repositorio
{
    public class RepositorioFuncionarios
    {
        ...
        {
            return Funcionarios.Where(x => x.Nome.ToLowerInvariant()
        ...
        public IList<Funcionario> FiltrarPorIdadeAproximada(int idade)
        {
            ...
            return Funcionarios.Where(x => ano - x.DataNascimento.Year <= idade + 5 
                                        && ano - x.DataNascimento.Year >= idade - 5).ToList();
        }        

        public double SalarioMedio(TurnoTrabalho? turno = null)
        {
            var funcs = turno == null ? Funcionarios : Funcionarios.Where(x => x.TurnoTrabalho == turno);
            return funcs.Average(x => x.Cargo.Salario);
        }

        public IList<Funcionario> AniversariantesDoMes()
        {
            int mes = DateTime.Today.Month;
            return Funcionarios.Where(x => x.DataNascimento.Month == mes).ToList();
        }
        ...
        {
            return Funcionarios.GroupBy(x => x.TurnoTrabalho)
                            .Select(x => new {
                                Turno = x.Key,
                                Quantidade = x.Count()}).ToArray();
        }
        ...
        public dynamic FuncionarioMaisComplexo()
        {
            return Funcionarios.Select(x => new
            {
                ...
                QuantidadeMesmoCargo = Funcionarios.Where(y => y.Cargo.Equals(x.Cargo)).Count()
